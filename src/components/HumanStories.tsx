    ...
    >
      ...
      <div className="w-full max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-10 items-stretch bg-white border border-[#E8EDF2] rounded-[32px] overflow-hidden p-6 md:p-8 shadow-[0_24px_64px_rgba(15,23,42,0.015)]">
        ...
        <div className="lg:col-span-6 relative aspect-[4/3] lg:aspect-auto lg:min-h-[480px] rounded-[24px] bg-slate-950 overflow-hidden flex items-center justify-center">
          ...
          {/* Floated Metric Glass Plate Card & Watch Story Button grouped on bottom-left */}
          <div className="absolute bottom-3 left-3 sm:bottom-6 sm:left-6 z-20 flex flex-col items-start gap-2 sm:gap-3">
            <AnimatePresence mode="wait">
              <motion.div
                key={activeIndex}
                initial={{ opacity: 0, y: -10 }}
                ...
                className="bg-[#0b0f17]/80 backdrop-blur-md border border-white/10 px-3 py-2 sm:px-4 sm:py-2.5 rounded-[12px] sm:rounded-[14px] text-white shadow-lg flex items-center gap-2 sm:gap-3"
              >
                <div className="text-lg sm:text-2xl font-black font-sans tracking-tight text-[#2BC48A]">
                  {activeStory.metric}
                </div>
                ...
                <div className="flex flex-col text-left">
                  <span className="text-[9px] sm:text-[10px] font-bold tracking-wider text-white uppercase leading-tight">
                    {activeStory.metricLabel}
                  </span>
                  <span className="text-[8px] sm:text-[9px] text-slate-300 font-medium opacity-80 mt-0.5">
                    Metric Index
                  </span>
                </div>
              </motion.div>
            ...
            {/* Floated "Watch the story" Button */}
            {activeStory.videoUrl && (
              <button
                onClick={() => setIsPlayingVideo(true)}
                className="flex items-center gap-2 sm:gap-2.5 bg-white p-2 sm:px-4 sm:py-2.5 rounded-full shadow-[0_8px_24px_rgba(0,0,0,0.15)] hover:shadow-[0_12px_32px_rgba(0,0,0,0.25)] transition-all duration-300 group cursor-pointer"
              >
                <div className="w-6 h-6 sm:w-7 sm:h-7 rounded-full bg-[#E5F7F0] flex items-center justify-center text-[#2BC48A] group-hover:scale-110 transition-transform duration-300 shrink-0">
                  <Play className="w-2.5 h-2.5 sm:w-3 sm:h-3 fill-current ml-0.5" strokeWidth={2.5} />
                </div>
                <div className="flex flex-col text-left">
                  <span className="text-[11px] sm:text-xs font-bold text-[#0F172A] leading-tight flex items-center">Watch the story</span>
                  <span className="text-[9px] sm:text-[10px] text-[#667085] font-medium mt-0.5">3:45 min</span>
                </div>
              </button>
            )}
          </div>
